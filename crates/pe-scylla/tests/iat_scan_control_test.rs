...
use pe_scylla::api::{IatScanner, ImportResolver, ResolvedImport};
...
struct NoResolver;
impl pe_scylla::api::ImportResolver for NoResolver {
    fn resolve(&self, _address: u64) -> Option<pe_scylla::api::ResolvedImport> {
        None
    }
}
...
fn remap_iat_references_repoints_code_to_scattered_slots() {
    for arch in [Arch::Bit64, Arch::Bit32] {
        ...
        let patched = pe_scylla::api::iat_scanner::remap_iat_references(&mut doc, &mapping)
            .expect("remap refs");
...
fn recover_dump_imports_reflects_overwritten_oft() {
    for arch in [Arch::Bit64, Arch::Bit32] {
        let doc = reflect_1a_doc(arch);
        let rec = pe_scylla::api::iat_fixer::recover_dump_imports(&doc, &ControlResolver)
