...
pub fn get_imports(
    ...
    get_imports_regions(pid, resolver, &[(iat_va, iat_size)])
}

/// Like [`get_imports`], but for a **list of IAT regions** — Scylla's
/// scattered-IAT support. A packed program can slice the IAT into several
/// non-contiguous regions; pass each `(va, size)` here and the resolved thunks
/// are merged into one tree (per-module, first-seen order).
pub fn get_imports_regions(
    pid: u32,
    resolver: &ProcessResolver,
    regions: &[(u64, usize)],
) -> Result<ImportsTree> {
    if regions.is_empty() {
        return Err(PeError::InvalidArgument(
            "get_imports: no IAT regions".into(),
        ));
    }
    ...
    for &(iat_va, iat_size) in regions {
        if iat_size == 0 {
            continue;
        }
        // Read the region in page-sized chunks, stopping at the first
        // unreadable page (like Scylla's `readMemoryPartlyFromProcess`): the
        // caller's size may overrun into guard/stack pages, and a real IAT
        // ends at a NULL terminator anyway.
