//! IAT scanning on a dumped process's [`PeDocument`] — Scylla's IAT
//! autosearch. Implemented as free functions over pe-edit's image model (the
//! [`IatScanner`] trait is implemented here for [`PeDocument`]).
...
use pe_edit::domain::section::{IMAGE_SCN_CNT_CODE, IMAGE_SCN_MEM_EXECUTE};
use pe_edit::domain::types::ptr_size;
use pe_edit::domain::{Arch, IatEntry, IatScan, PeDocument, Rva, ScanMethod, ScanOptions};
...
impl IatScanner for PeDocument {
    fn scan(&self, resolver: &dyn ImportResolver, options: &ScanOptions) -> Result<IatScan> {
        match options.method {
            ScanMethod::Resolver => scan_by_resolver(self, resolver, options),
            ScanMethod::CodeReference => scan_by_code_reference(self, resolver, options),
            ScanMethod::Reflection => scan_by_reflection(self, resolver, options),
        }
    }
}
...
fn scan_by_resolver(
    ...
    let (start, len) = scan_window(doc, options)?;
    ...
    for off in (0..len).step_by(psize) {
        ...
        let slot = doc.read(slot_rva, psize).ok().map(|b| read_thunk(b, psize));
        match slot {
...
fn scan_by_code_reference(
    ...
    for section in &doc.sections {
        ...
        while decoder.can_decode() {
            ...
            if slot_rva.get() % psize as u32 != 0 || !slot_in_data(doc, slot_rva) {
                continue;
            }
            if let Ok(bytes) = doc.read(slot_rva, psize) {
...
fn scan_by_reflection(
    ...
    if let Some(dd) = non_null_dir(doc, DataDirectoryIndex::Import) {
    ...
    if let Some(dd) = non_null_dir(doc, DataDirectoryIndex::Iat) {
        let mut entries = Vec::new();
...
fn non_null_dir(doc: &PeDocument, idx: DataDirectoryIndex) -> Option<DataDirectory> {
    doc.data_directory(idx)
        .ok()
        .copied()
...
fn slot_in_data(doc: &PeDocument, rva: Rva) -> bool {
...
/// image extent.
fn scan_window(doc: &PeDocument, options: &ScanOptions) -> Result<(Rva, usize)> {
    if let Some((rva, len)) = options.region {
        return Ok((rva, len));
    }
    let (s0, e0) = doc.sections.iter().fold((u32::MAX, 0u32), |(s0, e0), sec| {
...
pub fn remap_iat_references(doc: &mut PeDocument, mapping: &[(Rva, Rva)]) -> Result<usize> {
    if mapping.is_empty() {
        return Ok(0);
    }
