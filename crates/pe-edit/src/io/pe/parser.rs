...
/// Parse the DOS header. Exposed for the process-dump module (pe-scylla).
pub fn parse_dos(bytes: &[u8]) -> Result<DosHeader> {
...
/// Parse the COFF header at `off`. Exposed for the process-dump module (pe-scylla).
pub fn parse_coff(bytes: &[u8], off: usize) -> Result<CoffHeader> {
...
pub fn parse_section_headers(bytes: &[u8], off: usize, n: usize) -> Result<Vec<SectionHeader>> {
...
/// Exposed so the writer can check whether an existing directory still matches
/// the document's rich imports (and skip re-rendering when it does), and so
/// pe-scylla's dump/reflection recovery can re-parse a dumped image.
pub fn parse_imports_from_doc(doc: &PeDocument, dir_rva: Rva) -> Result<Vec<ImportDescriptor>> {
...
pub fn parse_thunks(doc: &PeDocument, thunk_rva: Rva, psize: usize) -> Result<Vec<ImportFunction>> {
    let mut out = Vec::new();
    let mut i: usize = 0;
...
pub fn parse_exports_from_doc(doc: &PeDocument, dd: DataDirectory) -> Result<Option<ExportTable>> {
...
pub fn read_cstring(doc: &PeDocument, rva: Rva) -> Result<String> {
    let mut out = Vec::new();
    for i in 0..4096u32 {
        let byte = doc.read(
            rva.checked_add(i)
...
pub fn parse_resources_from_doc(doc: &PeDocument, dd: DataDirectory) -> Result<ResourceDirectory> {
...
pub fn parse_relocations_from_doc(doc: &PeDocument, dd: DataDirectory) -> Result<RelocationTable> {
...
pub fn parse_tls_from_doc(doc: &PeDocument, dd: DataDirectory) -> Result<TlsDirectory> {
...
pub fn parse_load_config_from_doc(
    doc: &PeDocument,
    dd: DataDirectory,
